...
void JpsiPtExt(Bool_t IsOutput=0)
{
gStyle->SetOptStat(0);
    
Int_t nTrails = 5*pow(10,3);
Float_t dFitI = 0.5, dFitF = 16.;
Float_t dExtI = 16., dExtF = 40.;
        
//const Double_t Xbins[] = {0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 7., 9., 11., 13., 15., 18., 20., 22., 26., 30., 35., 40.};
const Double_t Xbins[] = {0., 1., 2., 3., 4., 5., 6., 8., 10., 12., 14., 16., 18., 20., 22., 26., 30., 35., 40.};
Int_t Nbinsx     = sizeof(Xbins)/sizeof(Double_t)-1;
cout << "Nbinsx = " << Nbinsx << endl;
    
const Double_t xbins[] = {0., 1., 2., 3., 4., 5., 6., 8., 10., 12., 14., 16.};
Int_t nbinsx     = sizeof(xbins)/sizeof(Double_t)-1;
cout << "nbinsx = " << nbinsx << endl;
    
const Double_t Ext_xbins[] = { 16., 18., 20., 22., 26., 30., 35., 40.};
...
TFile* infile = new TFile("final.root", "READ");
//infile->ls();
TDirectoryFile *Table1 = (TDirectoryFile*)infile->FindObjectAny("Table 3");
//Table1->ls();
...
Float_t p0,p1,p2;
p0=2.21862e+02, p1=9.50128e+00, p2=8.34651e-01; //p-Pb
...
for (Int_t i=0; i<nTrails; i++) {
 ...
 for(int j=0;j<Ext_nbinsx;j++){
  ...
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  leg->AddEntry(hData,Form("J/#psi, 2.03 < y_{cms} < 3.53"),"LP");
...
//     
  ...
  la.SetNDC();
  la.SetTextSize(0.05);
  la.SetTextAlign(13);//12
  la.DrawLatex(0.13,0.87,Form("Pb-p at #sqrt{s_{NN}} = 8.16 TeV"));
    
  if(IsOutput){
      TFile *outfile = new TFile(Form("output/WeightPtJpsiTsallis.root"),"RECREATE");
