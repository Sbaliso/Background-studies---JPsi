...
void JpsiPtExt_2_10_powlaw(Bool_t IsOutput=0)
{
gStyle->SetOptStat(0);
...
// p0=4.79978e+02, p1=9.10523e+00, p2=7.43153e-01; //p-Pb
 p0=4.25865e+02, p1=-4.56144e+00, p2=3.80754e+00; // Pb-p

for (Int_t i=0; i<nTrails; i++) {
    fFit[i] = new TF1(Form("fFit_%d",i+1), JpsiPt, dFitI, dFitF, 3);
    fFit[i]->SetParameters(p0,p1,p2);
    
    fExt[i] = new TF1(Form("fExt_%d",i+1), JpsiPt, dExtI, dExtF, 3);
...
//     
  ...
  la.DrawLatex(0.18,0.85,Form("Pb-p at #sqrt{s_{NN}} = 8.16 TeV"));
  la.DrawLatex(0.18,0.78,Form("-4.46 < y_{cms} < -2.96"));
  la.DrawLatex(0.21,0.71,"2-10 %");
    
  if(IsOutput){
      TFile *outfile = new TFile(Form("output/Pbp/powerlaw/WeightJpsiPt_2_10.root"),"RECREATE");
